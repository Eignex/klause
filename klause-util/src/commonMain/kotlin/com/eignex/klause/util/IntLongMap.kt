 ...
 */
class IntLongMap private constructor(private val backing: Backing, private val absent: Long) {
    /** Return the value stored for [key], or the map's absent sentinel. */
    operator fun get(key: Int): Long = backing.get(key, absent)

    /** Return whether [key] has an entry, independent of the absent sentinel. */
    fun contains(key: Int): Boolean = backing.get(key, MISSING) != MISSING

    /** Builders over parallel key/value arrays. */
    companion object {
