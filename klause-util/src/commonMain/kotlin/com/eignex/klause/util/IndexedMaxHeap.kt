 ...
 *
 * @property capacity Number of distinct ids the heap admits; ids run over `0..capacity-1`.
 */
class IndexedMaxHeap(val capacity: Int) {
    ...
    /** Return whether [id] is currently in the heap. */
    fun contains(id: Int): Boolean = pos[id] >= 0

    /** Return [id]'s stored key, whether or not it is currently in the heap. */
     ...
     *  equal-key ties resolve to the lowest id — the deterministic tie-break the variable pickers
     *  (ABS, DomWdeg) depend on after a key reset. Requires `size == capacity` —
