 ...
 */
class MinCostAssignment(private val numVars: Int, private val numValues: Int) {
     ...
     *
     * @property feasible Whether every variable could be matched.
     * @property cost Total cost of the matching, `0` when infeasible.
     * @property assignedValue Value index chosen per variable, empty when infeasible.
     */
    class Result(val feasible: Boolean, val cost: Long, val assignedValue: IntArray) {
        /** Canonical results that need no search. */
        companion object {
            /** The verdict for a subproblem that admits no complete assignment. */
