...
/** Correctness coverage for the remaining factor translations in [OrToolsModel]. */
class OrToolsFactorCoverageExtraTest {
    ...
    @Test fun `member picks one of the array values`() {
        val a = sat(problem(3, arrayOf(IntDomain(0, 3), IntDomain(0, 3), IntDomain(0, 3)),
            Member(xs = intArrayOf(0, 1), y = 2)))
        assertTrue(a.ints[2] == a.ints[0] || a.ints[2] == a.ints[1])
    }

    @Test fun `array max is the maximum`() {
        val a = sat(problem(3, arrayOf(IntDomain(0, 5), IntDomain(0, 5), IntDomain(0, 5)),
    ...
    @Test fun `argmax points at the first maximal position`() {
        val a = sat(problem(3, arrayOf(IntDomain(0, 5), IntDomain(0, 5), IntDomain(0, 1)),
    ...
    @Test fun `knapsack ties weight and profit sums`() {
        val a = sat(problem(4, arrayOf(IntDomain(0, 2), IntDomain(0, 2), IntDomain(0, 100), IntDomain(0, 100)),
    ...
    @Test fun `diffn separates two rectangles`() {
        val a = sat(problem(4, dom(4, 0, 3),
            Diffn(xs = intArrayOf(0, 1), ys = intArrayOf(2, 3), widths = intArrayOf(2, 2), heights = intArrayOf(2, 2))))
        val (x0, x1, y0, y1) = listOf(a.ints[0], a.ints[1], a.ints[2], a.ints[3])
    ...
    @Test fun `regular accepts its language`() =
        ...
        assertEquals(2, count(problem(2, dom(2, 1, 2), Regular(
            seq = intArrayOf(0, 1), numStates = 2, alphabetSize = 2,
            transitions = intArrayOf(2, 0, 2, 2), q0 = 1, accepting = intArrayOf(2)))))

    @Test fun `bin_packing fills load variables`() {
        val a = sat(problem(4, arrayOf(IntDomain(1, 2), IntDomain(1, 2), IntDomain(0, 100), IntDomain(0, 100)),
            BinPacking(bins = intArrayOf(0, 1), weights = intArrayOf(3, 4), mode = BinPacking.Mode.LoadVars,
                loadVars = intArrayOf(2, 3), numBins = 2, binOffset = 1)))
        val w = intArrayOf(3, 4)
        val load1 = (0..1).filter { a.ints[it] == 1 }.sumOf { w[it] }
        val load2 = (0..1).filter { a.ints[it] == 2 }.sumOf { w[it] }
        assertEquals(load1, a.ints[2]); assertEquals(load2, a.ints[3])
    }

    @Test fun `reified linear matches its predicate`() {
        val a = sat(mixedProblem(1, 1, arrayOf(IntDomain(0, 5)),
    ...
    @Test fun `reified pseudo-boolean matches its predicate`() {
        val a = sat(mixedProblem(3, 0, emptyArray(),
            ReifiedPseudoBoolean(auxBoolVar = 2, weights = intArrayOf(1, 1),
                literals = intArrayOf(Lit.make(0, true), Lit.make(1, true)), op = PbOp.GE, bound = 1)))
    ...
    @Test fun `reified cardinality matches its predicate`() {
        val a = sat(mixedProblem(3, 0, emptyArray(),
            ReifiedCardinality(auxBoolVar = 2, literals = intArrayOf(Lit.make(0, true), Lit.make(1, true)), min = 1, max = 2)))
        val n = (if (a.bools[0]) 1 else 0) + (if (a.bools[1]) 1 else 0)
        assertEquals(n in 1..2, a.bools[2])
    }

    @Test fun `product multiplies`() {
        val a = sat(problem(3, arrayOf(IntDomain(2, 2), IntDomain(3, 3), IntDomain(0, 20)),
            Product(a = 0, b = 1, result = 2)))
        assertEquals(6, a.ints[2])
    }

    @Test fun `pseudo-boolean enforces the weighted bound`() {
        val a = sat(mixedProblem(2, 0, emptyArray(),
            PseudoBoolean(weights = intArrayOf(1, 1), literals = intArrayOf(Lit.make(0, true), Lit.make(1, true)),
                op = PbOp.GE, bound = 1)))
