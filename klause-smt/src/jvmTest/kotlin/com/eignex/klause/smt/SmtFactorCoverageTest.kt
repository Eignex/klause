...
/** Correctness coverage for the global-constraint decompositions in [SmtTranslator]. */
class SmtFactorCoverageTest {
    ...
    @Test fun `cumulative is unsat when tasks cannot fit`() {
        ...
        assertTrue(SmtSolver(p).solve(SmtParams()) is SolveResult.Unsat)
    }
    ...
    @Test fun `set subset allows three configs per position`() = assertEquals(
        9,
        SmtSolver(boolProblem(4, SetBitsetSubset(intArrayOf(0, 1), intArrayOf(2, 3))))
            .enumerate(SmtParams()).toList().size,
    )

    @Test fun `set disjoint allows three configs per position`() = assertEquals(
        9,
        SmtSolver(boolProblem(4, SetBitsetDisjoint(intArrayOf(0, 1), intArrayOf(2, 3))))
            .enumerate(SmtParams()).toList().size,
    )

    @Test fun `set equal couples the indicators`() = assertEquals(
        4,
        SmtSolver(boolProblem(4, SetBitsetEq(intArrayOf(0, 1), intArrayOf(2, 3))))
            .enumerate(SmtParams()).toList().size,
    )
    ...
    @Test fun `mdd accepts the language`() = assertEquals(
        2,
        count(
            problem(
                2,
