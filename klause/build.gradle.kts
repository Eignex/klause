...
val checkPackageBoundaries = tasks.register("checkPackageBoundaries") {
    group = "verification"
    description = "Checks the production package dependency direction."
    inputs.files(
        fileTree("src") {
            include("**/*Main/**/*.kt")
        },
    ...
    doLast {
        val forbiddenImports = mapOf(
            "ir" to Regex(
                """^import com\\.eignex\\.klause\\.(solver|factor|propagation|backtrack|lp|localsearch|presolve|portfolio|theory|compile|formats)\\.""",
            ),
            "lowering" to Regex("""^import com\\.eignex\\.klause\\.(compile|formats)\\."""),
            "compile" to Regex("""^import com\\.eignex\\.klause\\.formats\\."""),
            "arithmetic" to Regex(
                """^import com\\.eignex\\.klause\\.(propagation|backtrack|localsearch|portfolio|presolve|solver\\.search)\\.""",
            ),
        )
        val violations = buildList {
            inputs.files.files.sorted().forEach { source ->
                val relative = source.invariantSeparatorsPath
                val packageName = source.useLines { lines ->
                    lines.firstOrNull { it.startsWith("package ") }?.removePrefix("package ")
                } ?: return@forEach
                val layer = forbiddenImports.keys.firstOrNull { packageName == "com.eignex.klause.$it" || packageName.startsWith("com.eignex.klause.$it.") }
                    ?: return@forEach
                val forbidden = forbiddenImports.getValue(layer)
                source.useLines { lines ->
                    lines.forEachIndexed { index, line ->
                        if (forbidden.containsMatchIn(line)) add("$relative:${index + 1}: $line")
                    }
                }
            }
        }
...
tasks.named("check") {
    dependsOn(checkPackageBoundaries)
}
