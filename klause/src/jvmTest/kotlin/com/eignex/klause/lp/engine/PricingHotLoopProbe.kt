...
fun main(args: Array<String>) {
    ...
    val validating = args.getOrNull(1) == "validate"
    val repetitions = if (validating) 2 else 30
    for (id in listOf("mps-adlittle", "mps-afiro")) {
        ...
        for (zeroCost in listOf(false, true)) {
            ...
            owner.use { solver ->
                repeat(if (validating) 0 else 60) { solver.solve() }
                var bytes = 0L
                var nanos = 0L
                var pivots = 0L
                var work = 0L
                ...
                repeat(repetitions) {
                    ...
                    for (j in variables.indices) {
                        check(result.primal[j].isFinite() && result.primal[j] in -1e-6..10.000001)
                        for (entry in pointers[j] until pointers[j + 1]) {
                ...
                if (validating) {
                    println("validation\t$id\t$zeroCost\t$results")
                } else {
                    println(
                        "pricing\t$id\t$zeroCost\t${source.rows}\t${source.columns}\t" +
                            "${bytes / repetitions.toDouble()}\t${nanos / repetitions.toDouble()}\t" +
                            "$pivots\t$work\t$samples\t$results",
                    )
                }
            }
        }
