...
class RoundAccumulator {
    ...
    /** Note an absolute cost seen this round; tracks the minimum for [RoundLog.bestCost]. */
    fun observeCost(cost: Double) {
        if (cost < best) best = cost
    }

    /** Freeze the round at [temperature]. Does not clear — call [clear] to start the next round. */
    fun snapshot(temperature: Double): RoundLog = RoundLog(
        proposed = proposed,
        accepted = accepted,
        costMean = if (count > 0) mean else 0.0,
        costVariance = if (count > 1) m2 / count else 0.0,
        bestCost = if (best.isFinite()) best else 0.0,
