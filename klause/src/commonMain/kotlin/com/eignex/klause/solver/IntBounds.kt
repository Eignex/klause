 ...
 *
 * An [IntDomain] is deliberately finite: it is the value set the CP search can branch on. This table
 * instead describes the model-level range a theory or relaxation sees. The finite endpoints stay in the
 * search-domain array and open sides are packed into two bitsets, so a million-variable problem does not
 * allocate a bound object or a [BigInteger] per column.
...
class IntBounds internal constructor(
    private val intDomains: Array<IntDomain>?,
    ...
    val size: Int get() = intDomains?.size ?: requireNotNull(lowerBounds).size
    ...
    fun lower(v: Int): Long {
        ...
        return lowerBounds?.get(v) ?: intDomains!![v].min
    }
    ...
    fun upper(v: Int): Long {
        ...
        return upperBounds?.get(v) ?: intDomains!![v].max
    }
    ...
    /** Internal constructors for finite-search and source-model storage layouts. */
    companion object {
        internal fun fromOpenSides(
