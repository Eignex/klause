...
internal fun SmtLib.Builder.digitizeWideInts(
    ...
    // The refutation over the open ranges lives in the deferred bounding, which substituting would
    // discard, so ask for it first. A real `unsat` beats a representable search every time.
    if (exactBoundsInfeasible(openRanges(inventedLo, inventedHi), rows)) return emptyMap()
...
/** The bounds as the model states them: a side the box invented reads back as infinite. */
private fun SmtLib.Builder.openRanges(inventedLo: BooleanArray, inventedHi: BooleanArray): Array<OpenIntBounds> =
    Array(nextInt) { v ->
        OpenIntBounds(
            if (inventedLo[v]) null else domainMin(v),
            if (inventedHi[v]) null else domainMax(v),
        )
    }
