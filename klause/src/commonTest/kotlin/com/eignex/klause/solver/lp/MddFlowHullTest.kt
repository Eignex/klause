...
class MddFlowHullTest {
    ...
    fun `cost-MDD flow hull gives the exact minimum path cost`() {
        ...
        assertEquals(2.0, sol.objectiveValue + r.objectiveConstant, eps, "minimum accepted-path cost is 2")
    }
    ...
    fun `randomized plain MDD flow hull matches the brute-force optimum`() {
        ...
        repeat(300) { _ ->
            ...
            assertEquals(
                ...
                eps,
                "MDD flow hull optimum ${sol.objectiveValue + r.objectiveConstant} != brute $opt",
            )
        }
