...
class LpReasonSoundnessTest {
    ...
    fun `lp learning preserves the optimum on random reified problems`() {
        ...
        repeat(300) {
            ...
            val autoParams = BacktrackParams(randomSeed = 7L, lubyRestartBase = 8L, lpConfig = LpConfig.AGGRESSIVE)
            when (val res = BacktrackSolver(p).minimize(objective, autoParams)) {
                is MinimizeResult.Optimal -> assertEquals(
                    bruteBest.toDouble(),
                    res.objective,
                    1e-9,
                    "lpConfig wrong optimum on instance #$it",
                )

                is MinimizeResult.Infeasible ->
                    assertEquals(Long.MAX_VALUE, bruteBest, "lpConfig Infeasible on brute-feasible instance #$it")

                else -> error("unexpected non-terminal lpConfig result $res on instance #$it")
            }
        }
