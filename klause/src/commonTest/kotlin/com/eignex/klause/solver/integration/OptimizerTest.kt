...
class OptimizerTest {
    ...
    fun `local search optimizer picks min weight single select`() {
        ...
        val sample = solver.minimize(objective, LocalSearchParams(maxFlips = 400L, randomSeed = 1L)).assignment
    ...
    fun `local search optimizer minimizes linear int cost`() {
        ...
        val sample = LocalSearchSolver(problem.bake())
            .minimize(objective, LocalSearchParams(maxFlips = 500L, randomSeed = 7L)).assignment
        assertNotNull(sample)
    ...
    fun `local search optimizer on all different`() {
        ...
        val sample = LocalSearchSolver(problem.bake())
            .minimize(objective, LocalSearchParams(maxFlips = 1_000L, randomSeed = 13L)).assignment
        assertNotNull(sample)
