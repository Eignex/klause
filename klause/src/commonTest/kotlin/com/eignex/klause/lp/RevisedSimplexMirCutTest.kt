...
class RevisedSimplexMirCutTest {
    ...
    @Test
    fun `mir cut fires on a fractional optimum and is valid`() {
        // max x  s.t. 2x <= 3, x in [0,5]  -> x = 3/2 fractional. MIR must cut it.
        val b = LpBuilder()
        ...
        val cuts = simplex.mirCuts(8)
        assertTrue(cuts.isNotEmpty(), "expected a MIR cut for the fractional optimum")
    ...
    @Test
    fun `randomized mir cuts never exclude an integer-feasible point`() {
        ...
        repeat(500) {
            ...
            // every cut. A violation would mean MIR removed a feasible integer point — unsound.
