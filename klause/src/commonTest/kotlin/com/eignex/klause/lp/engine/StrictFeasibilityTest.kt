...
class StrictFeasibilityTest {
    ...
    fun `expanded strict models decline admission before creating owners`() {
        ...
        val source = ExactLpModel(listOf(emptyList()), emptyList(),
            listOf(ExactLpColumn(ExactLpBounds(ExactLpSide(zero, strict = true), ExactLpSide(ExactLpNumber.of(1L))), integral = false)),
            emptyList(), ExactLpObjective(listOf(zero)))
        LpScopedSolver(LpExactState(source)).use { owner ->
            val result = refineLp(assertNotNull(owner.state.toWorkingModel()),
    ...
    fun `a positive auxiliary witness is sufficient without an objective certificate`() {
        ...
        val factory = object : LpEngineFactory by ProductionLpEngineFactory {
            override fun newPersistentSolver(model: LpModel, cancellation: Cancellation, refactorUpdateLimit: Int,
                iterationLimit: Int, workLimit: Long, trackDegeneracy: Boolean, pricing: LpPricingOptions): PersistentLpSolver {
                val engine = ProductionLpEngineFactory.newPersistentSolver(model, cancellation, refactorUpdateLimit,
                    iterationLimit, workLimit, trackDegeneracy, pricing)
                return object : PersistentLpSolver by engine {
                    override fun resolveBounds(allowance: LpFloatAllowance?): FloatLpResult? = engine.resolveBounds(allowance)?.let {
                        FloatLpResult(it.basis, it.objective, DoubleArray(model.m), it.primal, exactState = it.exactState)
                    }
                }
            }
        }
    ...
    fun `equivalent source replacement retains strict attempt spending`() {
        ...
        LpScopedSolver(LpExactState(source)).use { donor ->
            val first = refineLp(assertNotNull(donor.state.toWorkingModel()),
            ...
            LpScopedSolver(LpExactState(source)).use { recipient ->
                ...
                val repeated = refineLp(assertNotNull(recipient.state.toWorkingModel()),
    ...
    fun `signed strict rows retain closed source sides and origins`() {
        ...
        val model = ExactLpModel(
            ...
            listOf(zero, one),
            listOf(
                ExactLpColumn(ExactLpBounds(ExactLpSide(zero), ExactLpSide(one)), origin = ExactLpNumber.of(10L), integral = false),
                ExactLpColumn(ExactLpBounds(ExactLpSide(zero)), integral = false),
                ExactLpColumn(ExactLpBounds(ExactLpSide(zero)), integral = false),
            ),
            List(2) { ExactLpRow(strict = true) },
            ExactLpObjective(listOf(ExactLpNumber.of(6L), zero, zero), constant = ExactLpNumber.of(3L),
                scale = ExactLpNumber.of(2L), externalConstant = one),
        )
        ...
        assertEquals((BigFraction.ofLong(6L) * x + BigFraction.ofLong(3L)) *
            BigFraction.ofLong(2L).reciprocal() + BigFraction.ONE, assertNotNull(result.witness).objective)
    ...
    fun `strict cycle conflict contains only source equations and active witnesses`() {
        ...
        val source = ExactLpModel(
            listOf(listOf(ExactLpEntry(0, one), ExactLpEntry(1, minusOne)),
        ...
        LpScopedSolver(LpExactState(source)).use { owner ->
            ...
            val result = refineLp(assertNotNull(state.toWorkingModel()),
                LpRefinementRequest(owner, owner.refinementCache, LpRefinementLimits(maxRounds = 0)))
            ...
            val rho = assertNotNull(support.conflictMultipliers)
    ...
    fun `strict zero rows and fixed sides are refuted exactly`() {
        ...
        for (structural in listOf(false, true)) {
            val source = ExactLpModel(
                ...
                listOf(ExactLpColumn(ExactLpBounds(ExactLpSide(zero, strict = true), ExactLpSide(zero)), integral = false)),
    ...
    fun `failed auxiliary engine callbacks close the child and retain spending`() {
        ...
        LpScopedSolver(LpExactState(source), context = LpSolveContext(engineFactory = factory)).use { owner ->
            assertFailsWith<IllegalStateException> {
                refineLp(assertNotNull(owner.state.toWorkingModel()),
