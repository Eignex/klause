...
- `[CP]` LCG: have int-domain factors emit atom-lit antecedents to take full advantage of the LCG infrastructure. Full LCG engine is now in place: bound-atom registry (`state.atomVarGe(v, k)` / `atomVarLe(v, k)`), `state.litTrue` / `litFalse` / `pinLit` dispatch helpers, per-atom watchers (`atomWatchersByLit`), `atomsByIntVar` triggers atom truth recomputation on every int tighten via `propagateAtomsForVar`, `dirtyAtomFactors` drained in `runToFixpoint`, atom-aware `maxLevelForVars` / `maxLevelForClause`, snapshot/restore that drops post-snapshot atoms and refreshes pre-snapshot atom truth from restored domains. `Clause` refactored to use the dispatch helpers — atom-lit clauses unit-propagate via `pinLit` which re-derives the corresponding int-bound tighten on the underlying var. `ConflictAnalyzer` handles atom-var ids natively. Tested end-to-end: an atom-lit clause `[[v0 ≥ 5], [v1 ≥ 7]]` correctly unit-propagates `v1.min ≥ 7` when `v0.max` is tightened below 5. Remaining: have int-domain factors emit atom-lits in their antecedent compositions instead of bool-lit unions via `state.composeIntVarAntecedents` (both are sound; atom-lits give per-bound granularity, bool-lit unions are coarser but simpler to construct).
