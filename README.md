...
- `[CP]` Remaining propagator-strength work: incremental STR2 / GAC-Schema for `Table` (current is GAC but re-scans every tuple per call — STR2 is a constant-factor speedup); sweep-line / energetic edge-finding for `Cumulative` beyond time-tabling (Schutt-Feydy-Stuckey 2009).
