...
- `[CP]` Richer propagation-graph cores in BacktrackSolver. The result types carry an optional `UnsatCore` (subset of `Problem.factors` ids); klause-z3 and klause-smt populate it via tracked assertions, and `BacktrackSolver` lifts the failing factor id from `PropagationResult.Unsat.conflictFactors` whenever root-level propagation rules the problem out. Today's BacktrackSolver core names only the *single* factor that returned `false` from `propagate`; full "every factor whose firing contributed" attribution requires a reason trail, landing alongside LCG-style no-good learning. (klause-logicng unsat-core extraction is intentionally not on this list — it would need a CNF-clause → factor-id back-map plumbed through the bit-blast pipeline, and our internal optimisers cover the use case.)
