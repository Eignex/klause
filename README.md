...
- `[CP]` LCG: clause-database management. First-1UIP analysis and persistent learned-clause storage are live — clauses are kept on `PropagationState`, indexed in `boolWatchersByLit` with virtual factor ids, asserted via `PropagationSession.addLearnedClause`. Remaining: LBD (Literal Block Distance) scoring per learned clause, restart-driven forgetting (LBD-keep-best or geometric decay) to bound memory growth, and conflict-graph reasons for non-Clause factors (Cardinality, Linear, AllDifferent need `conflictReason` + antecedent-recording implementations) so LCG strength extends beyond pure SAT shapes. Restart-frequency tuning (Luby base) on top of the existing schedule is also a natural fit.
