...
- `[CP]` LCG: have int-domain factors emit atom-lit antecedents (via `state.atomVarGe(v, k)` / `atomVarLe(v, k)`) instead of the coarser bool-lit unions they currently build via `state.composeIntVarAntecedents`. Both are sound; atom-lits give per-bound granularity and let the analyzer / minimizer resolve over specific bound facts.
