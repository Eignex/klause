...
- `[CP]` LCG: extend conflict reasons to int-domain factors. Bool-pinning factors (Cardinality, PseudoBoolean, ReifiedCardinality, ReifiedPseudoBoolean, Xor) now emit clause-form `conflictReason` and record antecedents. Remaining: int-domain reasons for `Linear`, `AllDifferent`, `GlobalCardinality`, `Element`, etc. — needs an analyzer extension that resolves over `intMinReason` / `intMaxReason` trails, not just bool implications.
