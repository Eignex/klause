...
- `[CP]` Factor: stronger circuit / subcircuit propagation. The `Circuit` and `Subcircuit` factors ship with bounds + self-loop shaving for propagation and an O(n) walk-the-successor LS scoring. Missing: union-find-of-fixed-segments sub-cycle detection (prune `succ[i] = j` when `j` is reachable from `i` via fixed predecessors before all `n` are visited). The full Hamiltonian property currently relies on LS scoring; a strong propagator would make BacktrackSolver competitive on TSP-like instances.
