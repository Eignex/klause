...
- `[Format/FlatZinc]` Set-var follow-ups now that the indicator-decomposition path is in: (a) array-of-set-of-int handling so `all_disjoint` / `set_partition_into` can dispatch; (b) `set_in` for var-int elements (today only constant-int elements work — needs per-element reified equality channel); (c) reified set ops (`set_subset_reif`, `set_eq_reif`, `set_union_reif`, ...) — currently only `set_in_reif` is wired. Stronger than these is the offload to MZN's stdlib decomposition: leaving a predicate undeclared in `redefinitions.mzn` lets MZN expand it to lower-level set ops we already handle.
