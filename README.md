...
- Perf: O(1) deltaIfBoolFlipped on Clause. Cardinality and AllDifferent now use precomputed IntIntMap lookups; Clause keeps the O(arity) full-scan fallback because the obvious O(1) variant assumed a watched-literal invariant (if any literal is true then at least one watch is true) that the random-walk property test surfaces breaking. Fix path: strengthen the rewatch logic in applyBoolFlip first, then port the watch-based delta in.
