...
- Meta-optimizer: extend ALNS beyond the Ropke-Pisinger 2006 baseline. Shipped: random + worst-objective + adjacency-related destroy, `InnerLsRepair` parameterised by flip budget (default palette: standard / quick / deep), two independent roulette-wheel bandits over destroy and repair, ILS-derived acceptance criteria. What's missing: activity-biased and currently-violated destroy (needs cross-call state from the inner solver), problem-specific destroy (cumulative time-window slides), non-LS repair heuristics (greedy construction, regret-based), and Thompson-sampling-bandit replacement once kumulant `BernoulliSum` lands.
