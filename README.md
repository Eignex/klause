...
- `[CP]` LCG: extend antecedent recording to remaining int-domain factors. End-to-end LCG works for `Linear` / `ReifiedLinear` chains via bool-literal antecedent threading: `PropagationState.intMinAntecedents` / `intMaxAntecedents` store reasons on int-bound tightenings, `propagateLinearBounds` chains through other vars' int trail on every tighten, `ReifiedLinear` composes aux-pin antecedents from involved vars' int facts, and `ConflictAnalyzer.analyzeDecisionConflict` learns from decision-vs-prior-pin contradictions (so chains that pre-imply bools at lower levels get resolved during minimization). Remaining: (a) record antecedents in `AllDifferent` (Régin matching + SCC pruning), `GlobalCardinality` (max-flow + SCC), `Element`, `Cumulative` / `Disjunctive`, `Sort`, `Table`, `Inverse`, etc. — each tighten/exclude site needs to compose its reason from the involved variables' int antecedents; (b) (optional, deeper) virtual int-bound literals (`[x ≥ k]`, `[x ≤ k]`) for sharper per-bound granularity, so analyzer resolution can target specific int-domain facts rather than coarse bool-literal unions.
