...
- `[CP]` Propagator-strength follow-ups beyond the current GAC / competitive baseline: incremental STR2 / GAC-Schema for `Table` (constant-factor speedup over today's per-call re-scan — needs snapshot-aware per-factor payload that survives push/pop, since refPayload deliberately drifts); sweep-line / energetic edge-finding for `Cumulative` past time-tabling (Schutt-Feydy-Stuckey 2009).
