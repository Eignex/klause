...
- `[CP]` Propagator strength follow-ups (current set is GAC or competitive for every weak global): incremental STR2 / GAC-Schema for `Table` (current is GAC but re-scans every tuple per call — STR2 is a constant-factor speedup); sweep-line / energetic edge-finding for `Cumulative` beyond the time-tabling baseline (Schutt-Feydy-Stuckey 2009).
- `[CP]` Conflict reasons (`Factor.conflictReason`) for the hole-pruning propagators (AllDifferent, GlobalCardinality, Regular, AllDifferentExceptZero, Inverse) — these need Hall-set / sweep-prefix reasons since bound atoms alone aren't sound, plus interior-hole atoms in the literal alphabet (`atomVarNe` / `atomVarEq`). Bound-only propagators (Linear, LexLess, Diffn, Knapsack, Cumulative, NValue, Among, Count, Sort, ArgMinMax, SymmetricAllDifferent) done.
