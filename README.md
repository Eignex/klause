...
- Local search: richer aspiration policies on `TabuFilter`. `OrImprovesBestEver` admits tabu moves that beat `LocalSearchState.bestCostSeen` (now persisted in `WarmState` so the watermark survives `Session.minimize` boundaries). Missing: probabilistic / temperature-based aspiration in the spirit of `AcceptanceCriterion.SimulatedAnnealing` so tabu moves are accepted with decaying probability rather than a hard threshold. The dynamic-tenure side has `randomBand` and `linearGrowth` factories.
