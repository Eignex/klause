...
- `[CP]` Factor: cumulative(starts, durations, resources, capacity). The headline scheduling global — at every time point the sum of resource consumption stays under capacity. Time-tabling propagator is the standard implementation. Already referenced under `[Format/FlatZinc]` for native FlatZinc dispatch, but missing as a klause-DSL constraint too.
- `[CP]` Factor: disjunctive(starts, durations). Tasks may not overlap in time. Special case of cumulative with capacity = 1, but a dedicated propagator (edge-finding, not-first/not-last) is much stronger on disjunctive-only instances.
...
- `[API]` Per-backend Session subclasses with native incremental engines. `LogicNGSession` shipped — one MiniSat instance reused across calls, learned conflict clauses persist, per-call assumptions passed as MiniSat literal assumptions. Z3 path superseded by the planned [JavaSMT backend](https://github.com/sosy-lab/java-smt) which gives a uniform incremental interface across Z3/CVC5/MathSAT5/Bitwuzla/SMTInterpol/Yices2/Princess.
...
- `[Backend]` klause-smt module on top of JavaSMT. Scaffold shipped — `SmtSolver` handles `solve()` + per-call assumptions, SMTInterpol default backend (pure-Java, ~2.6 MB total module weight after excluding Princess). Klause-z3 stays alive alongside for users who want the lean single-backend native Z3 path. Remaining: `samples()` / `enumerate()` (model-blocking via JavaSMT), `Optimizer.minimize()` (via `OptimizationProverEnvironment`, Z3/MathSAT-only), `Session` subclass with persistent context.
