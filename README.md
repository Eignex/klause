...
- Local search: adaptive noise and parameter control. WalkSat's noise, ProbSat's exponent, DDFW's increment are hardcoded. A picker that plateau-detects via kumulant EwmaMean and bumps the parameter up during stalls, decays back during progress, is +10-30% on hard random instances (Hoos, ParamILS / SMAC line of work). Pairs with the kumulant heuristic-learning items.
...
- Perf (post-benchmark): O(1) deltaIfBoolFlipped on Clause. The two-watched-literal scheme is set up for `isViolated` but `deltaIfBoolFlipped` currently calls `anyLitTrueAfterFlip` which scans every literal. Should leverage the existing watches: pre-flip status from `isViolated`, post-flip status from checking only the watch literals (with rewatch lookup amortized).
- Perf (post-benchmark): O(1) deltaIfBoolFlipped on Cardinality. Currently scans every literal to find the one referencing `boolVar`. Precompute a `boolVar → contribution` lookup at construction so the delta becomes a single hash lookup + arithmetic.
