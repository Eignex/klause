...
- Restart-level heuristic portfolio bandit. UCB1 or Thompson sampling over a configurable palette of variable and value heuristic configs; updates between Luby restarts. Uses kumulant's BernoulliSum per strategy.
...
- Meta-optimizer: extend ALNS beyond the Ropke-Pisinger 2006 baseline. Shipped: random + worst-objective + adjacency-related + currently-violated destroy plus `activityBiased(session)` driven by cumulative per-variable touch counts persisted in `WarmState`; `InnerLsRepair` parameterised by flip budget (default palette: standard / quick / deep) plus `GreedyConstructionRepair` for myopic value-by-value fill; `Bandit` interface implemented by both `RouletteWheelBandit` and kumulant-backed `ThompsonBandit`; ILS-derived acceptance criteria; opt-in `session` constructor field that routes repair through a `LocalSearchSession` so DDFW weights and activity counts accumulate across iterations. What's missing: problem-specific destroy (cumulative time-window slides), regret-based / best-improving construction repairs.
...
- Local search: refine EWMA-mode noise control. `NoiseController` has an opt-in `ewmaAlpha` that switches improvement detection from "below all-time low" to "below kumulant-EwmaMean smoothed cost"; missing: integration into the adaptive strategy constructors so callers configure EWMA via `AdaptiveWalkSat(ewmaAlpha = ...)` rather than reaching into the controller directly, plus an auto-tune that picks alpha from the problem's flip budget.
