...
- Online heuristic learning via Bayesian linear models on [kumulant](https://github.com/eignex/kumulant). LinUCB / Linear Thompson Sampling: per (var, value) feature vector φ (domain size, activity, depth, last-conflict distance, value offset, recency, propagator failure rate, ...); full-covariance Gaussian posterior `N(μ, Σ)` over weights; Thompson sampling draws `w̃` and picks the value maximising `σ(w̃ᵀφ)`. Generalises across pairs via shared weights — no per-pair cold start. Per-variable kumulant Moments (mean, variance, skewness, kurtosis of conflict depth) for cumulant-based variable ordering. EwmaMean / DecayingMean for dom/wdeg with principled decay. Extensions: distributional regression with kumulant DDSketch for risk-aware picking; hierarchical Bayesian priors shared across instances within a problem class — klause's structural differentiator over one-shot solvers (OR-tools / Chuffed throw heuristic state away between instances).
- Extend kumulant with `BayesianLinearStat`. Gaussian posterior over weights, conjugate Bayesian updates per observation, Thompson sampling primitive, mergeable across observation streams. Klause is its first consumer; useful standalone for online experiment analysis. ~200 lines of math-with-tests.
- Restart-level heuristic portfolio bandit. UCB1 or Thompson sampling over a configurable palette of (variable, value) heuristic configs; updates between Luby restarts. Uses kumulant's `BernoulliSum` per strategy.
- Multi-core LS portfolio for the MiniZinc Challenge Local Search track. N workers with different (strategy, seed) combinations; shared kumulant stats in Relaxed concurrency mode track per-strategy success; best-feasible-solution sharing across workers. The track's small competitor field (Yuck, OscaR/CBLS, iZplus) plus klause's existing LS engine make this the most realistic medal target.
- Multi-core CP portfolio for the Free / Parallel tracks. Workers with different heuristic configs; shared Beta-Bernoulli posteriors merged via kumulant's snapshot+merge primitives; objective-bound sharing for optimization. Pairs with the kumulant learning items.
- Cross-sample / cross-instance posterior persistence on `Session`. Pairs with the incremental-solving Session API. Within a problem class, instance N starts from instance N-1's learned posterior; the heuristic gets better with use. OR-tools and Chuffed don't do this — it's where klause's sampling-first positioning translates into a competitive edge.
...
- Floats: refactor bucketing from problem-level to per-backend. Add a Float sort to Problem with FloatInterval domain; each backend declares its capability. Z3 uses native Real; LocalSearch picks float-aware moves; Backtrack does interval branching; bit-blaster and LogicNG apply a FloatLowering bucketing pass at the boundary. Current "schema-level bucketing" is the wrong layer.
