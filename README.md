...
- `[CP]` LCG: extend conflict reasons to non-Clause factors. Add `conflictReason` + antecedent-recording propagation on `Cardinality` (literal-based, natural first), `Linear`, `AllDifferent`, etc. Until then the analyzer falls back to chronological backtrack on non-Clause failures.
