...
- `[CP]` Stronger native propagators for the globals shipped with weak (singleton-violation-check) propagation. Every MZN global already has a Factor + FZN dispatch arm; upgrade in priority order: Pesant layered-DAG for `Regular`; sweep-line for `Diffn`; STR2/STR3 / GAC-Schema for `Table`; flow-based for `Knapsack`; bound-consistency for `Inverse`; lex-conflict propagator for `LexLess` / `LexLesseq`. (`AllDifferent` and `GlobalCardinality` now ship Régin matching/SCC and max-flow/SCC GAC respectively.)
