...
- Meta-optimizer: extend ALNS beyond the Ropke-Pisinger 2006 baseline. Shipped: random + worst-objective + adjacency-related + currently-violated destroy plus `activityBiased(session)` that reads per-variable recency from a `LocalSearchSession`; `InnerLsRepair` parameterised by flip budget (default palette: standard / quick / deep) plus `GreedyConstructionRepair` for myopic value-by-value fill; two independent roulette-wheel bandits over destroy and repair; ILS-derived acceptance criteria. What's missing: problem-specific destroy (cumulative time-window slides), regret-based / best-improving construction repairs, ALNS by-default running over a session (currently each iteration is a fresh call so `activityBiased` requires explicit wiring), and Thompson-sampling-bandit replacement once kumulant `BernoulliSum` lands.
- Meta-optimizer: ILS variants beyond the shipped Lourenço-Martin-Stützle 2003 baseline (Improving / BetterOrEqual / RandomWalk / SimulatedAnnealing acceptance, adaptive perturbation strength, optional population of incumbents picked at random per restart, uniform-crossover restarts when population ≥ 2). What's still missing: basin-hopping-style large-jump perturbation (the existing adaptive ramp already approximates this), and objective-biased crossover (currently uniform 50/50).
