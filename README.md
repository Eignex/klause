...
- `[API]` UNSAT cores in klause-logicng + richer propagation-graph cores in BacktrackSolver. The result types carry an optional `UnsatCore` (subset of `Problem.factors` ids); klause-z3 and klause-smt populate it via tracked assertions, and `BacktrackSolver` lifts the failing factor id from `PropagationResult.Unsat.conflictFactors` whenever root-level propagation rules the problem out. Remaining: (1) klause-logicng — lifting MiniSat's `unsatCore()` needs a CNF-clause → factor-id back-map plumbed through `LogicNGTranslator`'s bit-blast pipeline (the indirection is the only reason this wasn't done in the same pass as Z3/SMT). (2) Propagation-graph attribution for BacktrackSolver — today's core names only the factor that returned `false` from `propagate`; richer "every factor whose firing contributed" requires a reason trail, landing alongside LCG-style no-good learning. (3) Z3's `Optimize` solver doesn't surface cores under `MinimizeResult.Infeasible` (would require dropping the optimization wrapper).
