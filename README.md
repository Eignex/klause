...
- Meta-optimizer: extend ALNS with a richer destroy palette and multiple repair operators. The shipped `Alns` covers the Ropke-Pisinger 2006 baseline: random + worst-objective destroy, single-repair (the inner LS engine under pinned assumptions), roulette-wheel adaptive bandit, and the ILS-derived acceptance criteria. What's missing: activity-biased and problem-aware destroy (cumulative time-window slides, conflict-related variable selection), multiple repair heuristics (greedy, regret-based, best-improving), and Thompson-sampling-bandit replacement once kumulant `BernoulliSum` lands.
- Meta-optimizer: ILS variants beyond the basic incumbent-anchored restart. The shipped `IteratedLocalSearchRestart` covers the Lourenço-Martin-Stützle 2003 baseline (Improving / BetterOrEqual / RandomWalk acceptance, adaptive perturbation strength). What's still missing: simulated-annealing acceptance (temperature-scheduled rejection of worse optima), basin-hopping-style large-jump perturbation, and a population-of-incumbents variant.
