...
- `[CP]` LCG: extend conflict reasons to non-Clause factors. CDCL core is complete on the Clause side (1UIP, self-subsuming-resolution minimization, persistent learned clauses with watchers, LBD scoring, restart-driven forgetting via `maxLearnedClauses` / `lbdGlueThreshold`, cascading-CDB loop). Remaining: `conflictReason` + antecedent-recording propagation on `Cardinality` (literal-based, natural first), `Linear`, `AllDifferent`, etc. Until then the analyzer falls back to chronological backtrack on non-Clause failures.
...
- `[CP]` Stronger native propagators for the globals shipped with weak (singleton-violation-check) propagation. Every MZN global already has a Factor + FZN dispatch arm; upgrade in priority order: Régin GAC for `AllDifferent` and `GlobalCardinality`; Pesant layered-DAG for `Regular`; sweep-line for `Diffn`; STR2/STR3 / GAC-Schema for `Table`; flow-based for `Knapsack`; bound-consistency for `Inverse`; lex-conflict propagator for `LexLess` / `LexLesseq`.
- `[LS]` Multi-core LS portfolio finishing touches for the MZN Challenge LS track. Coordination layer (Session-per-worker, race-first-feasible, fan-in sample flow, cooperative cancellation) is in place. Remaining: best-feasible sharing as warm-start hints, shared kumulant stats in Relaxed mode for a restart-level bandit, worker-config factory handing each worker a distinct `(strategy, seed)`.
...
- `[LS]` ILS: basin-hopping-style large-jump perturbation (current adaptive ramp approximates) and multi-parent / linkage-aware crossover beyond shipped Uniform / BetterBiased mixing.
...
- `[LS]` Richer VNS beyond the shipped k-tuple Compound cycle: VND (exhaust each k before promotion), per-level neighbourhood operators (swap-only at N2, 3-opt at N3), skewed-VNS accepting mild worsening at higher k.
- `[LS]` Problem-aware move generation for globals not yet covered: cumulative time-window slides, lex-aware moves for `lexLeq` / `lexLt`, reified-factor diversification. Extension point is per-factor `proposeRepairMoves` (AllDifferent / Linear / Product / Circuit / Cumulative / Disjunctive already specialised).
- `[LS]` Apply LinUCB / Linear Thompson Sampling inside LS itself (not just CP). Kumulant `BayesianLinearRegression` with custom prior + population fit is in place; concrete targets:
...
- `[Format/FlatZinc]` Native set variables via bool-indicator decomposition. Today the FZN parser declines `var set of ...` cleanly; the target is to claim native sets in `redefinitions.mzn` and at FZN-load time lower each set var to a contiguous bool range with a side `setIndicatorMap` so the FZN writer can reconstruct `{a,b,c}` output from indicator bools. Every `set_*` predicate (`set_in`, `set_subset`, `set_card`, `set_union`, `set_intersect`, `set_diff`, `set_eq`, `all_disjoint`, `set_partition`) lowers to existing `Clause` / `Cardinality` / `PseudoBoolean` / `ReifiedLinear` factors over the indicator bools — unit-propagation strength is what falls out, no new factor types needed.
- `[Perf]` Native bitset set-propagators where benchmarking shows bool-decomposition's per-bool propagator dispatch is the bottleneck. For sets with large universes (>256 elements) and many set algebra operations per propagation cycle, bitset throughput on `(LB, UB)` representations beats N independent bool propagator calls by a constant factor — the `Bits` primitive is in place; the cost is engine plumbing (set domain arrays in `PropagationState`, dedicated move type, snapshot extension) that wasn't justified at unit-prop strength. Revisit only with profiling data.
