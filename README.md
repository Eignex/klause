...
- `[CP]` LCG: bound atoms and analyzer substitution to finish lazy clause generation. Foundation landed: `PropagationState.intMinAntecedents` / `intMaxAntecedents` arrays, antecedent-passing `tightenIntMin` / `tightenIntMax` / `excludeIntValue` overloads, snapshot/restore. `ReifiedLinear` composes aux-pin antecedents from int-trail antecedents of involved vars; `propagateLinearBounds` threads the reif aux as `extraLit` and chains through other vars' int antecedents on every tighten. Remaining: (a) virtual int-bound literals (`[x ≥ k]`, `[x ≤ k]`) with lazy allocation and per-atom levels/antecedents, so the analyzer's resolution loop can walk int trail entries directly; (b) extend `ConflictAnalyzer.minimize` / `isRedundant` to *substitute* level-N implied vars via their antecedents (currently only drops literals whose antecedents are subsumed by the clause or by level-0 facts); (c) wire antecedents into the rest of the int-domain factors (`AllDifferent` / `GlobalCardinality` Régin matching + SCC pruning, `Element`, `Cumulative`, `Disjunctive`, `Sort`, `Table`, etc.).
