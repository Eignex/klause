...
- `[CP]` Propagator-strength follow-ups beyond the current GAC / competitive baseline: incremental STR2 / GAC-Schema for `Table` (constant-factor speedup over today's per-call re-scan); sweep-line / energetic edge-finding for `Cumulative` past time-tabling (Schutt-Feydy-Stuckey 2009).
- `[CP]` Interior-hole literal alphabet (`atomVarNe` / `atomVarEq` atoms) + Hall-set / sweep-prefix `conflictReason` overrides for the hole-pruning propagators (AllDifferent, GlobalCardinality, Regular, AllDifferentExceptZero, Inverse, Member). Bound-only propagators already covered.
- `[CP]` Singleton arc consistency / root-value probing. Generalise the existing bool-only failed-literal probe in `Problem.bake` to int vars: pin `x = v`, propagate, learn `x ≠ v` on Unsat. Needs `Implied` / `Assumptions` extended to carry bound-tightenings (not just exact pins); opt-in flag, budget-bounded.
