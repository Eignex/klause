...
- Local search: extend cost shaping into the pre-feasibility phase. Today the configured `CostShaping` only steers the post-feasibility greedy descent — the strategy (WalkSat / ProbSat / DDFW) still drives toward `cost == 0` looking only at constraint violations. Threading shaping into `state.breakScore` / per-strategy candidate scoring would let the LS engine bias the *whole* search toward objective-favourable basins. Bigger refactor; needs a way to inject the objective into the strategy's scoring path. The post-feasibility shaping shipped (Linear / Saturating / SquareRoot variants).
...
- Local search: richer aspiration / tenure policies on `TabuFilter`. The current `OrImproving` aspiration admits a tabu move if it strictly lowers the *current* violation count; the literature "improves the best ever observed" needs a tracked best-cost across the search session. Dynamic-tenure presets (random within a band, growing during stalls) would also be useful — the `dynamicTenure: (Long) -> Int` hook is in place but no named policies ship yet. The `TabuFilter` component itself, replacing 6 inline tabu blocks across WalkSat / ProbSat / DDFW / CcaWalkSat / SimulatedAnnealing / adaptive variants, has shipped.
