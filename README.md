...
- `[CP]` LCG: have int-domain factors emit atom-based antecedents in place of bool-literal unions. Full LCG infrastructure now landed — virtual int-bound atoms `[x ≥ k]` / `[x ≤ k]` are lazily allocated in `PropagationState`'s atom registry past the bool var space, with per-atom value / level / antecedents snapshotted from the current `intDomains` / `intLevel` / `intMin/MaxAntecedents`. `ConflictAnalyzer` recognises atom-var ids (via `state.isAtomVar(v)`) and dispatches between `boolLevel` / `boolAntecedents` and `atomLevel` / `atomAntecedents` in `ingestReason`, `drainSeenAsLeaves`, `minimize`, `isRedundant`, `lbdOf`, and `backjumpLevelOf`. Bool-literal LCG is functional end-to-end across every int-domain factor that prunes / tightens; the remaining sharpening work is having factors emit `state.atomLitGe(v, k)` / `atomLitLe(v, k)` directly in their antecedent compositions (currently they use unioned bool lits via `state.composeIntVarAntecedents`), so the analyzer can resolve over specific bound facts rather than coarse var-wide unions.
