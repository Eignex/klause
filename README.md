...
- Online heuristic learning via [kumulant](https://github.com/eignex/kumulant). Per (var, value) Beta-Bernoulli + Thompson sampling for value selection; per-variable Moments (mean, variance, skewness, kurtosis of conflict depth) for cumulant-based variable ordering; EwmaMean / DecayingMean for dom/wdeg with principled decay; OLS over per-variable feature vectors for combined heuristics. This is klause's structural differentiator over OR-tools / Chuffed, which throw heuristic state away between instances.
