...
- `[CP]` Conflict reasons (`Factor.conflictReason`) for the propagators that prune interior values (AllDifferent, Cumulative beyond bound-only, GlobalCardinality, Regular) — these need Hall-set / sweep-prefix reasons since bound atoms alone aren't sound. Bound-only propagators (Linear, LexLess, Diffn, Knapsack) already done.
