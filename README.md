...
- Local search: best-ever-cost aspiration on `TabuFilter`. `OrImproving` admits a tabu move if it strictly lowers the *current* violation count; the literature "improves the best ever observed" needs a best-cost tracker across the session. The dynamic-tenure side has `randomBand` and `linearGrowth` factories now.
