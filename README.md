...
- `[CP]` LCG: extend antecedent recording to remaining int-domain factors. End-to-end LCG works for `Linear` / `ReifiedLinear` / `AllDifferent` / `GlobalCardinality` chains via bool-literal antecedent threading: `PropagationState.intMinAntecedents` / `intMaxAntecedents` store reasons on int-bound tightenings, factors compose antecedents from the involved variables' int trails on every prune, and `ConflictAnalyzer.analyzeDecisionConflict` learns from decision-vs-prior-pin contradictions (so chains that pre-imply bools at lower levels get resolved during minimization). Remaining: (a) record antecedents in the rest of the int-domain factors — `Element`, `Cumulative` / `Disjunctive`, `Sort`, `Table`, `Inverse`, `Circuit`, `Subcircuit`, scheduling propagators, etc.; (b) (optional, deeper) virtual int-bound literals (`[x ≥ k]`, `[x ≤ k]`) for sharper per-bound granularity, so analyzer resolution can target specific int-domain facts rather than coarse bool-literal unions.
