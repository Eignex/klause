...
- `[Perf]` Bitset-backed `IntDomain` for narrow spans (≤ 64 → single Long; ≤ a few hundred → LongArray). O(1) contains / excludeValue replacing the sorted-IntArray holes path. Multi-week: every propagator's hot path reads IntDomain and an immutable representation means every mutation copies the bitmask. Add as a third internal variant alongside contiguous and sparse-IntArray, dispatched via a private factory.
- `[Perf]` Move-pool inlining: pack `Move.BoolFlip` / `Move.IntSet` into a `Long` (1-bit kind, 31-bit var, 32-bit value); store the `MoveSink` primitive lane as a `LongArray`, keep a parallel ArrayList for variable-length `Move.Compound` parts. Touches every strategy that consumes `MoveSink.list`.
