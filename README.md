...
- `[CP]` Stronger native propagators for the remaining weak globals: Pesant layered-DAG for `Regular` (memory-efficient bitmask layers to keep per-call allocation bounded); incremental STR2 / GAC-Schema for `Table` (current `Table` is already GAC but re-scans every tuple per call); chained-prefix tightening for `LexLess` / `LexLesseq` beyond the first non-equal index.
- `[CP]` Conflict reasons (`Factor.conflictReason`) on the remaining propagators still on the all-decisions fallback — tightens learned-clause LBD and unlocks shorter backjumps. Most globals that prune interior values (AllDifferent, Cumulative beyond bound-only, GlobalCardinality, Regular) need Hall-set / sweep-prefix reasons; bound-only propagators (Linear, LexLess, Diffn, Knapsack done) just need their bound-atom set.
