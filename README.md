...
- `[Format/FlatZinc]` `.ozn` evaluator so `klause-fzn-cli` can render the MZN-side output expression natively without delegating to `solns2out`. Scope: the full MZN expression language as it appears in `output [...]` (array comprehensions over int/bool/set/string, `if-then-else`, `show` / `show2d` / `show3d` / `show_int(width, …)`, `fix`, `array1d`/`array2d`/`array3d` reshaping, arithmetic on params, set displays). MiniZinc itself emits this subset from any user model. The current `klause.msc` keeps `needsSolns2Out: true` so MZN's `solns2out` handles it; the goal is to make klause self-sufficient. Pair with the enum-mapping item below — `.ozn` is also where MZN enum tag names appear.
...
- `[Format/FlatZinc]` Remaining unsupported FZN predicates discovered via [`klause-mzn-lib/scripts/verify-corpus.sh`](klause-mzn-lib/scripts/verify-corpus.sh) over the MZN Challenge corpus (300 problems, 273/300 ok) and `libminizinc/tests/spec` (147 models, 142 ok): `set_le` / `set_lt` (lex-of-sorted-elements; see the `set_le/set_lt` bullet in the set-var follow-ups), `array_set_element` / `array_var_set_element` (element on an array of set constants / vars), `float_eq` / `float_le` / `float_lt_reif` / `float_eq_reif` / `float_min` / `float_max` / `float_times` / `float_lin_lt` / `float_lin_lt_reif` (bucket-aware lowering for the non-linear and missing comparison cases). Run `verify-corpus.sh` periodically against a fresh corpus snapshot to surface new gaps.
