...
- `[Perf]` `Clause`-specific O(1) break/make update via per-clause `numTrueLits` tracking (YalSAT / NuWLS style). `boolBreakCount` / `boolMakeCount` are already maintained eagerly inside `applyBoolFlip`, but the brute-force per-factor update is O(Σ arity²) per flip. With `numTrueLits` in `intPayload`, Clause flips touch at most 2 vars (the new and old critical literals) at O(1) each — biggest remaining LS-SAT speedup.
