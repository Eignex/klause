...
- `[CP]` Factor: circuit(succ) / subcircuit(succ). Hamiltonian cycle through all vars (`succ[i]` is the next node after `i`). Routing / TSP backbone; standard propagator detects forbidden sub-cycles and prunes domains. Already referenced under `[Format/FlatZinc]`.
