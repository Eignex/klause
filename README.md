...
- `[API]` UNSAT cores in the remaining backends. The result types now carry an optional `UnsatCore` (subset of `Problem.factors` ids); klause-z3 populates it via tracked assertions on UNSAT. Remaining: klause-logicng can lift MiniSat's `unsatCore()` once the bit-blasted clause → factor-id map is wired through Z3Translator's split; klause-smt has `prover.getUnsatCore()` available on tracked solvers; `BacktrackSolver` needs the propagation engine to carry the failing factor id through `PropagationResult.Unsat` (currently only the conflict vars / levels are surfaced).
