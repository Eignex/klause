...
- Local search: auto-tune `ewmaAlpha` from problem characteristics. The opt-in EWMA-mode in `NoiseController` is now plumbed through `AdaptiveWalkSat` / `AdaptiveProbSat` / `AdaptiveDdfw` constructors; what's missing is a heuristic that picks `ewmaAlpha` from problem size / flip budget so callers don't need to tune it.
...
- Local search: aspiration menu on `TabuFilter` is `AllowAllWhenAllTabu`, `OrImproving`, `OrImprovesBestEver`, `Probabilistic(rate)`, `Cooling(initialTemp, coolingRate, minTemp)` (SA-style decaying admission); dynamic-tenure side has `randomBand` and `linearGrowth`.
