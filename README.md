...
- `[Perf]` Cardinality two-watched-literal extension. Clauses now use the [Factor.initialBoolWatchers] / [PropagationState.boolWatchersByLit] / [PropagationState.moveBoolWatcher] machinery for `k+1`-style watched-literal wakeup; same scheme extends naturally to `Cardinality` (watch `min+1` literals so the propagator only fires when one of them flips). Smaller win than Clauses (Cardinality is less common) but the data structure is already in place.
