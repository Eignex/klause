...
- `[CP]` Propagator-strength follow-ups beyond the current GAC / competitive baseline: sweep-line / energetic edge-finding for `Cumulative` past time-tabling (Schutt-Feydy-Stuckey 2009). STR2 Table now incremental via `SnapshottablePayload`.
...
- `[CP]` Interior-hole SAC follow-up to the new bound-SAC root probe. `probeIntBounds` lifts min and max via assumption-bound tightenings; probing values strictly between bounds (and learning `x ≠ v` for interior holes) needs `Implied` / `Assumptions` extended to carry interior-hole sets in addition to the bound fields already added.
