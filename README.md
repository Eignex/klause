...
- Local search: problem-aware move generation for global constraints. AllDifferent now proposes up to 4 reservoir-sampled unused targets (was 1); Linear and Product already snap to exact-target values; the engine supports `Move.Compound` so factors can propose multi-variable atomic transitions (value-swap pairs, pair-flips). Still missing: AllDifferent emitting value-swap Compound candidates by default, cumulative time-window slides, and lex-aware moves for `lexLeq` / `lexLt`. Per-factor `proposeRepairMoves` is the extension point.
