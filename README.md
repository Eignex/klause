...
- `[CP]` LCG: (optional, deeper) virtual int-bound literals (`[x ≥ k]`, `[x ≤ k]`) for sharper per-bound granularity. Bool-literal LCG is functional end-to-end across every int-domain factor that prunes / tightens: `Linear`, `ReifiedLinear`, `AllDifferent`, `GlobalCardinality`, `AllEqual`, `AllDifferentExceptZero`, `Among`, `ArgMinMax`, `ArrayMinMax`, `BinPacking`, `Circuit`, `Subcircuit`, `Count`, `Cumulative`, `Disjunctive`, `Inverse`, `Knapsack`, `LexLess`, `Member`, `Monotone`, `NValue`, `Product`, `Sort`, `SymmetricAllDifferent`, `Table`. Each factor's tighten / exclude calls compose antecedents from involved vars' int trails via `state.composeIntVarAntecedents`; `ConflictAnalyzer.analyzeDecisionConflict` learns from decision-vs-prior-pin contradictions; `PropagationState.intMinAntecedents` / `intMaxAntecedents` store the per-bound reasons. Remaining (separate architecture work): virtual bool atoms `[x ≥ k]` allocated lazily, with per-atom value / level / antecedents stored on PropagationState past the bool var space, plus analyzer extension to walk atom trail entries. This sharpens reasoning over individual bound facts (vs the current union-of-bool-lits attribution) and unblocks LCG-style learning when int decisions are on the trail.
