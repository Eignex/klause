...
- `[CP]` SAC budgeting follow-ups past `probeBudgetPerVar` / `probeTotalBudget`: dom/wdeg-prioritised probe ordering (try high-impact vars first), and randomised tie-breaking when budget is hit mid-pass so reruns don't always cut the same tail.
