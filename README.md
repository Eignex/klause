...
- `[CP]` Search: last-conflict and impact-based value selection. The richer-conflict callback (`VariableHeuristic.onConflict(varRef, conflictBools, conflictInts)`) added for VSIDS already carries the per-conflict reason set both heuristics need; last-conflict just remembers the most-recent conflict variable and prioritises it on the next pick, impact-based scores each (var, value) pair by reduction in remaining-domain product. Both fit alongside [Vsids] in `Heuristics.kt`.
...
- `[CP]` Search: dom/wdeg variable ordering. Each constraint accumulates a weight on failure; pick the variable with the highest weight/domain-size ratio. Same scaffolding as [Vsids] — bump per-factor weights from `onConflict(varRef, _, _)` using the conflict's factor ids (already available via `PropagationResult.Unsat.conflictFactors`), then pick by `(Σ weight of constraints touching v) / (current domain size)`.
