...
- `[CP]` SAC budgeting / nested-fixed-point heuristics: today bound-SAC (`probeIntBounds`) and hole-SAC (`probeIntHoles`) iterate to a global fixed point at bake time, which is `O(Σ |dom(v)| × propagate)` per outer pass. A bake-time budget and per-var caps would let very wide domains opt into SAC without pathological construction cost.
