...
- Cross-sample and cross-instance posterior persistence via backend-specific Session subclasses that hold real cross-call state (learned clauses, kumulant posteriors, warm-start last solution). `LocalSearchSession` now implements `Session<LocalSearchParams>` and carries DDFW factor weights + per-variable activity recency across calls; what's still pending is LogicNG/Z3 session subclasses that expose native incremental engines, and cross-*instance* persistence so problem-class N starts from class N-1's learned state.
...
- Meta-optimizer: extend ALNS beyond the Ropke-Pisinger 2006 baseline. Shipped: random + worst-objective + adjacency-related + currently-violated destroy plus `activityBiased(session)` driven by cumulative per-variable touch counts persisted in `WarmState`; `InnerLsRepair` parameterised by flip budget (default palette: standard / quick / deep) plus `GreedyConstructionRepair` for myopic value-by-value fill; two independent roulette-wheel bandits over destroy and repair; ILS-derived acceptance criteria; opt-in `session` constructor field that routes repair through a `LocalSearchSession` so DDFW weights and activity counts accumulate across iterations. What's missing: problem-specific destroy (cumulative time-window slides), regret-based / best-improving construction repairs, and Thompson-sampling-bandit replacement once kumulant `BernoulliSum` lands.
- Meta-optimizer: ILS variants beyond the shipped Lourenço-Martin-Stützle 2003 baseline (Improving / BetterOrEqual / RandomWalk / SimulatedAnnealing acceptance, adaptive perturbation strength, optional population of incumbents picked at random per restart, crossover restarts with Uniform / BetterBiased value mixing when population ≥ 2). What's still missing: basin-hopping-style large-jump perturbation (the existing adaptive ramp already approximates this), and multi-parent / linkage-aware crossover.
...
- Local search: thread `CostShaping` into the remaining strategies. The engine now exposes `LocalSearchState.shapedBreakScore` (LinearObjective fast path) and WalkSat + CcaWalkSat consume it, so their pre-feasibility picks are objective-biased under a `Linear` shaping. ProbSat's `(eps + brk)^(-cb)` exponent and DDFW's weighted-break sum need bespoke shaped variants; their math doesn't accept negative shaped values directly. Non-linear `Objective` subtypes also skip shaping (would require apply-revert per evaluation).
...
- Local search: per-session best-cost aspiration on `TabuFilter`. The shipped `OrImprovesBestEver` tracks `LocalSearchState.bestCostSeen` (monotonically decreasing, preserved across restart epochs); a longer-lived watermark surviving across `Session.minimize` calls would let aspiration consult cross-call history. The dynamic-tenure side has `randomBand` and `linearGrowth` factories.
- Local search: variable neighborhood search (VNS) meta-heuristic. Cycle through neighborhood operators of increasing size when LS gets stuck — single-var moves → 2-var Compound swaps → 3-var moves → larger blocks. ILS's `adaptivePerturbation` ramp covers the *perturbation*-strength angle; VNS would cycle the *move-proposal* shape that the strategy iterates over. Building blocks are in place (`Move.Compound`, AllDifferent value-swaps) — what's missing is a strategy that explicitly cycles neighborhoods between stagnation episodes.
