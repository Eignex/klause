...
- Local search: cooling-temperature tabu aspiration. The shipped `Probabilistic(rate)` admits tabu moves at a constant rate; an SA-style variant with decaying acceptance probability (mirroring `AcceptanceCriterion.SimulatedAnnealing`) would tighten tabu over time. Tabu aspiration menu otherwise has `AllowAllWhenAllTabu`, `OrImproving`, `OrImprovesBestEver`, `Probabilistic`; dynamic-tenure side has `randomBand` and `linearGrowth`.
