...
- `[CP]` Cross-instance posterior persistence on top of the LinUCB/TS lane: pooled-prior store on disk that kumulant's `fitPopulationPrior` reads/writes between runs. The infrastructure on the LS side is sketched under `[LS+CP]` already; the CP-search analog reuses the same store.
