...
- `[CP]` LCG: extend engine to handle atom-lits in learned clauses. Atom infrastructure landed: lazy registry with snapshot/restore, `state.atomVarGe(v, k)` / `atomVarLe(v, k)` allocation, per-atom value/level/antecedents derived from int trail at allocation time. `ConflictAnalyzer` handles atom-var ids natively in `ingestReason`, `drainSeenAsLeaves`, `minimize`, `isRedundant`, `lbdOf`, `backjumpLevelOf`. Bool-literal LCG is functional end-to-end across every int-domain factor; atom-lit antecedents work at the analyzer level. The next sharpening step: extend `Clause` (and watched-literal infrastructure, `pinBool`, `moveBoolWatcher`) to recognise atom-var ids so learned clauses containing atom-lits can be added back via `addLearnedClause` and watched/propagated normally. Today `Clause` indexes `state.boolValues` directly by var id (sized `numBoolVars`), so atom-lits in a clause would read out-of-bounds; this needs a `state.litTrue(lit)` / `state.assignLit(lit, ant)` abstraction that dispatches between bool and atom, plus widening `boolWatchersByLit` past `2*numBoolVars`. Until that lands, factors emit bool-lit unions via `state.composeIntVarAntecedents` (the union of involved vars' int-trail antecedents) — sound and end-to-end, but coarser than per-bound atom resolution.
