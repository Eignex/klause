...
- `[CP]` Remaining propagator-strength work: multi-Long bitmask Pesant for `Regular` DFAs with > 64 states (today's GAC kicks in for ≤ 64); incremental STR2 / GAC-Schema for `Table` (current is GAC but re-scans every tuple per call); chained-prefix tightening for `LexLess` / `LexLesseq` beyond the first non-equal index.
