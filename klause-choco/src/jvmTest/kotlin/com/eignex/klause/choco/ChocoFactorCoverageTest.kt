...
class ChocoFactorCoverageTest {
    ...
    @Test fun `set subset allows three configs per position`() =
        assertEquals(9, ChocoSolver(boolProblem(4, SetBitsetSubset(intArrayOf(0, 1), intArrayOf(2, 3))))
            .enumerate(ChocoParams()).toList().size)

    @Test fun `set disjoint allows three configs per position`() =
        assertEquals(9, ChocoSolver(boolProblem(4, SetBitsetDisjoint(intArrayOf(0, 1), intArrayOf(2, 3))))
            .enumerate(ChocoParams()).toList().size)

    @Test fun `set equal couples the indicators`() =
        assertEquals(4, ChocoSolver(boolProblem(4, SetBitsetEq(intArrayOf(0, 1), intArrayOf(2, 3))))
            .enumerate(ChocoParams()).toList().size)
    ...
    @Test fun `mdd accepts the language`() =
        // Accepts seq[0] = 1 with seq[1] free → 2 words.
