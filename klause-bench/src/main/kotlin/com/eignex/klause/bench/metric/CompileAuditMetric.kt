...
@Serializable
internal data class AuditFamily(
    val family: String,
    val instances: Int,
...
@Serializable
internal data class AuditResults(
...
internal object CompileAuditMetric {
    fun run(refs: List<ProblemRef>) {
        ...
        val rows = try {
            refs.map { ref ->
                pool.submit<AuditRow> {
                    audit(
                        ref,
                        runner,
                        nativeSet,
                        fznCli.takeUnless { skipIngest },
                        ingestCap,
                    )
                }
            }
        ...
        val families = rows.groupBy { it.family }.map { (fam, rs) ->
            AuditFamily(
                fam,
                rs.size,
                rs.count { it.status == "COMPILE_ERROR" },
                rs.count { it.ingest == "crashed" },
                rs.filter { it.status == "OK" }.let { ok ->
                    if (ok.isEmpty()) {
                        0.0
                    } else {
                        ok.sumOf { it.coverage } /
                            ok.size
                    }
                },
            )
        ...
        Reports.writeMarkdown(
            "build/compile-audit-report.md",
            markdown {
                h1("MiniZinc compile audit")
                para(
                    "${rows.size} instances, ${rows.count { it.status == "COMPILE_ERROR" }} compile errors, " +
                        "${rows.count { it.ingest == "crashed" }} ingest crashes.",
                )
                h2("By family")
                table(
                    listOf("family", "instances", "compile-err", "ingest-crash", "avg coverage"),
                    families.map {
                        listOf(
                            it.family,
                            it.instances,
