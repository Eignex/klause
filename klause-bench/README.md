...
| `kind=cop\|csp` | keep optimization (COP) or satisfaction (CSP) problems — judged from the resolved objective (MiniZinc `solve minimize/maximize`, OPB `min:`, etc.), so it filters after resolution |
