...
- **Non-MiniZinc formats** (XCSP3 / SMT-LIB / DIMACS / OPB — no `.mzn` to hand to `minizinc`) are solved by **klause only**, via `klause-cli` on the original file. There is no external reference for these (the in-process Choco/OR-Tools/Yuck/LogicNG adapter modules were removed in the subprocess refactor).
