...
class SolverTest {
    ...
    fun solvesSimple3SatInstance() {
        ...
        val sample = solver.sample(maxFlips = 10_000, randomSeed = 7).first()
    ...
    fun samplesAreUniqueByDefault() {
        ...
        val solver = Solver(problem, maxFlipsBeforeRestart = 10)
        val samples = solver.sample(maxFlips = 5_000, randomSeed = 1).take(10).toList()
    ...
    fun exactlyOneFactorYieldsAllThreeSolutions() {
        ...
        val solver = Solver(problem)
        val samples = solver.sample(maxFlips = 5_000, randomSeed = 13).take(10).toList()
    ...
    fun duplicatesAllowedWhenDistanceZero() {
        ...
        val problem = Problem(3, 0, emptyArray(), listOf(factor))
        val solver = Solver(problem)
        val samples = solver.sample(maxFlips = 5_000, randomSeed = 13, minHammingDistance = 0).take(20).toList()
    ...
    fun rollingWindowAllowsReuseAfterRotation() {
        ...
        val solver = Solver(problem)
        val samples = solver.sample(maxFlips = 10_000, randomSeed = 21, recentWindow = 2).take(20).toList()
